...
CRED(USER,KEY,GRANT)
 ...
 I $G(GRANT)="" S GRANT="V"
 N GRSTR S GRSTR=$G(^TT("GRANTS",$G(USER),$G(KEY)))
 Q (GRSTR[GRANT)
...
AUTH(UN,PW)
 N RETVAL S RETVAL=0
 I ($G(^TT("USERS",UN))=UN)&($G(^TT("USERS",UN,"PASSWORD"))=($$PWIXF^ROOKAUTH(PW))) S RETVAL=1
 Q RETVAL

CREATE(UN,PW,EN,FN,MI,LN)
 TS
 S ^TT("USERS",UN)=UN
 S ^TT("USERS",UN,"PASSWORD")=$$PWIXF^ROOKAUTH(PW)
