...
START
 ...
 D MSGBOX^TTUI("Log started for "_$$GETEXTVAL^TTFORM(.FA,"CUSTOMER")_".",0,"TIME COLLECTION")
CANCELSTART
 Q
...
STOP
 ...
 S ^TT("ENTR",CUSTCODE,ENTRIDX,"UNITS")=$$HOURS^TTDATE(START,STOP)
 S ^TT("ENTR",CUSTCODE,ENTRIDX,"DATE")=$ZDATE(START)
 S ^TT("ENTR",CUSTCODE,ENTRIDX,"ITEM")=TASK
 S ^TT("ENTR",CUSTCODE,ENTRIDX,"BILLED")="NO"
 S ^TT("ENTR",CUSTCODE,ENTRIDX,"CUSTCODE")=CUSTCODE
 S ^TT("ENTR",CUSTCODE,ENTRIDX,"UNITPRICE")=^TT("CUSTOMER",CUSTCODE,"RATEHOUR")
 S ^TT("ENTR",CUSTCODE,ENTRIDX,"UNITTYPE")="HOUR"
 S ^TT("CUSTOMER",CUSTCODE,"ENTRIDX")=^TT("CUSTOMER",CUSTCODE,"ENTRIDX")+1
 D MSGBOX^TTUI("Log stopped for "_^TT("CUSTOMER",CUSTCODE)_".",0,"TIME COLLECTION")
 Q

CANCEL
 N LOGIDX,LOGREC,CUSTCODE
 S LOGIDX=$$CHOOSE(.LOGREC)
 S CUSTCODE=^TT("LOG",LOGIDX,"CUSTCODE")
 K ^TT("LOG",LOGIDX)
 D MSGBOX^TTUI("Log cancelled for "_^TT("CUSTOMER",CUSTCODE)_".",0,"TIME COLLECTION")
 Q
...
CREATE(CUSTCODE,TASK)
 ...
 S ^TT("LOG",IDX,"USERNAME")=TTUSER
 S ^TT("INOUT",TTUSER,$H)=TASK
