...
INVOICETOP
 ...
 S MA("ITEMS","MANUAL BILLING ENTRY","KEY")="XKINV:M"
 S MA("ITEMS","MANUAL BILLING ENTRY")="D CREATE^TTENTRY"
...
QUITIGLS
 S RESP=$$CONFIRM^TTUI("REALLY QUIT IGLS?")
 I RESP="Y" D
 . D RESET^TTRNSI
 . HALT
 Q
...
INIT
 S TTHELP="D KBHELP^TT"
 U $P:(CONVERT:NOWRAP)
 D INIT^TTRNSI
 D ADDACCEL^%TERMIO("KEY_F9","D QUITIGLS^TT","F9","QUIT IGLS")
 D ADDACCEL^%TERMIO("KEY_F1","D HELP^TT","F1","HELP")
 D NOSCROLLREGION^TTRNSI
 D SETCOLOR^TTRNSI("WHITE","BLACK")
 D CLEAR^TTRNSI
 D SCROLLREGION^TTRNSI(3,%IOCAP("ROWS")-2)
 ...
 D LOCATE^TTRNSI(1,%IOCAP("COLUMNS")-11)
 W $G(TTUSER,"NO USER")
 D SETCOLOR^TTRNSI("GREEN","BLACK")
 D LOCATE^TTRNSI(%IOCAP("ROWS")-1,1)
 F I=1:1:%IOCAP("COLUMNS") D LOCATE^TTRNSI(2,I) W BOX("H")
 D LOCATE^TTRNSI(23,1)
 F I=1:1:%IOCAP("COLUMNS") D LOCATE^TTRNSI(%IOCAP("ROWS")-1,I) W BOX("H")
 D LOCATE^TTRNSI(3,1)
 Q
